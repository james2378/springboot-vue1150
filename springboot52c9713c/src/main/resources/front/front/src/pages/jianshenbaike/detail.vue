...
<div>
	<div :style='{"padding":"20px 7%","margin":"0px auto","borderColor":"#ddd","borderRadius":"0px","background":"none","borderWidth":"0 0 1px","width":"100%","borderStyle":"solid"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'Ξ'" :style='{"fontSize":"14px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/jianshenbaike"><a>{{item.name}}</a></el-breadcrumb-item>
	...
	<div class="detail-preview" :style='{"padding":"0 7%","margin":"40px auto","flexWrap":"wrap","display":"flex","width":"100%","position":"relative","justifyContent":"space-between"}'>
		<div class="attr" :style='{"minHeight":"480px","padding":"0","margin":"0","flexWrap":"wrap","background":"none","display":"flex","width":"55%","position":"relative","justifyContent":"space-between","order":"2"}'>
			...
			<div class="info" :style='{"width":"100%","padding":"0px","margin":"0","background":"#fff","flex":"1","order":"2"}'>
				...
				<div class="item" :style='{"padding":"0 10px","margin":"0 0 0px 0","borderColor":"#ddd","background":"none","borderWidth":"0 0 1px","display":"flex","borderStyle":"dashed","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#999","textAlign":"right","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}'>文章简介</div>
					<div  :style='{"padding":"8px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#666","flex":"1","height":"auto"}'>{{detail.wenzhangjianjie}}</div>
				</div>
				<div class="item" :style='{"padding":"0 10px","margin":"0 0 0px 0","borderColor":"#ddd","background":"none","borderWidth":"0 0 1px","display":"flex","borderStyle":"dashed","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#999","textAlign":"right","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}'>健身类型</div>
					<div  :style='{"padding":"8px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#666","flex":"1","height":"auto"}'>{{detail.jianshenleixing}}</div>
				</div>
				<div class="item" :style='{"padding":"0 10px","margin":"0 0 0px 0","borderColor":"#ddd","background":"none","borderWidth":"0 0 1px","display":"flex","borderStyle":"dashed","justifyContent":"spaceBetween"}'>
				...
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#333","borderRadius":"4px","background":"#009cf550","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('jianshenbaike','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#333","borderRadius":"4px","background":"#f5340050","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('jianshenbaike','删除')" @click="delClick">删除</el-button>
					<!-- hasChat 无 -->
				</div>
			</div>
		</div>
		...
		<div class="detail2" :style='{"border":"0px solid #ddd","padding":"0px","margin":"40px 0","borderRadius":"0px","background":"#fff","flex":"1","width":"65%","clear":"both","order":"50"}'>
			<div :style='{"border":"1px solid #ddd","padding":"10px","margin":"0 0 10px 0","borderRadius":"4px","background":"#fcfcfc"}'>
				<div :style='{"color":"#333","fontSize":"16px","fontWeight":"600"}'>文章内容</div>
			</div>
			<div :style='{"background":"#FFF"}' v-html="detail.wenzhangneirong"></div>
		</div>
  ...
  export default {
    ...
    methods: {
      ...
      storeup(type) {
        if (type == 1 && !this.isStoreup) {
          ...
          this.$http.post('storeup/add', this.storeupParams).then(res => {
            if (res.data.code == 0) {
			  ...
			  this.$http.post('jianshenbaike/update', this.detail).then(res => {});
              this.$message({
                type: 'success',
                message: '收藏成功!',
                duration: 1500,
        ...
        if (type == -1 && this.isStoreup) {
          this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'jianshenbaike', userid: localStorage.getItem('frontUserid')}}).then(res => {
      ...
      nextClick(page) {
		...
		// 修改
		editClick(){
			this.$router.push(`/index/jianshenbaikeAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		delClick(){
			this.$confirm('是否删除此健身百科？')
			  .then(_ => {
			    this.$http.post('jianshenbaike/delete', [this.detail.id]).then(res => {
			      if (res.data.code == 0) {
			        this.$message({
			          type: 'success',
			          message: '删除成功!',

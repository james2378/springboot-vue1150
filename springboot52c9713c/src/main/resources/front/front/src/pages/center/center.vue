...
<div class="center-preview" :style='{"padding":"0 7%","margin":"20px auto 40px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative","justifyContent":"space-between"}'>
	...
	<div class="info" :style='{"padding":"10px","boxShadow":"0 0px 0px rgba(0, 0, 0, 0.3)","margin":"20px 3% 0 0","borderColor":"#eee","borderRadius":"0px","background":"#ffffff","borderWidth":"4px 2px 1px","width":"25%","borderStyle":"solid","height":"auto"}'>
		...
		<div :style='{"padding":"0 20px","borderColor":"#efefef","margin":"0 auto","borderWidth":"0 0 1px 0","display":"flex","width":"90%","lineHeight":"40px","borderStyle":"solid","height":"auto"}' v-if="userTableName=='yonghu'">
			<span class="icon iconfont icon-shouye-zhihui" :style='{"color":"#333","padding":"0 5px","fontSize":"14px"}'></span>
			<div :style='{"color":"#333","fontSize":"14px"}'>用户电话</div>
			<div :style='{"fontSize":"14px","color":"#333","textAlign":"right","flex":1}'>{{sessionForm.yonghudianhua}}</div>
		</div>
		<div :style='{"padding":"0 20px","borderColor":"#efefef","margin":"0 auto","borderWidth":"0 0 1px 0","display":"flex","width":"90%","lineHeight":"40px","borderStyle":"solid","height":"auto"}' v-if="userTableName=='yonghu'">
			<span class="icon iconfont icon-shouye-zhihui" :style='{"color":"#333","padding":"0 5px","fontSize":"14px"}'></span>
			<div :style='{"color":"#333","fontSize":"14px"}'>余额</div>
			<div :style='{"fontSize":"14px","color":"#333","textAlign":"right","flex":1}'>{{sessionForm.yue}}</div>
		</div>
		<div :style='{"padding":"0 20px","borderColor":"#efefef","margin":"0 auto","borderWidth":"0 0 1px 0","display":"flex","width":"90%","lineHeight":"40px","borderStyle":"solid","height":"auto"}' v-if="userTableName=='yonghu'">
			<span class="icon iconfont icon-shouye-zhihui" :style='{"color":"#333","padding":"0 5px","fontSize":"14px"}'></span>
			<div :style='{"color":"#333","fontSize":"14px"}'>会员等级</div>
			<div :style='{"fontSize":"14px","color":"#333","textAlign":"right","flex":1}'>{{sessionForm.huiyuandengji}}</div>
		</div>
    ...
    <el-tabs tab-position="left" :style='{"width":"72%","margin":"20px 0 0","flexWrap":"wrap","background":"#fff","flex":"1","display":"flex"}' @tab-click="handleClick">
      <el-tab-pane label="个人中心">
        <el-form class="center-preview-pv" ref="sessionForm" :model="sessionForm" :rules="rules" label-width="180px">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='yonghu'" label="用户电话" prop="yonghudianhua">
            <el-input v-model="sessionForm.yonghudianhua" placeholder="用户电话" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='yonghu'" label="余额" prop="yue">
            <el-input v-model="sessionForm.yue" placeholder="余额" readonly></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='yonghu'" label="会员等级" prop="huiyuandengji">
            <el-select v-model="sessionForm.huiyuandengji" placeholder="请选择会员等级" disabled>
              <el-option v-for="(item, index) in dynamicProp.huiyuandengji" :key="index" :label="item" :value="item"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='yonghu'" label="会员折扣" prop="huiyuanzhekou">
            <el-select v-model="sessionForm.huiyuanzhekou" placeholder="请选择会员折扣" disabled>
              <el-option v-for="(item, index) in dynamicProp.huiyuanzhekou" :key="index" :label="item" :value="item"></el-option>
            </el-select>
          </el-form-item>
...
<script>
  ...
  export default {
    ...
    created() {
		...
		if ('yonghu' == this.userTableName) {
			this.$set(this.sessionForm, 'yue', null);
		}
		if ('yonghu' == this.userTableName) {
		...
		if ('yonghu' == this.userTableName) {
			this.$set(this.rules, 'yue', [{ required: false, validator: this.$validate.isNumber, trigger: 'blur' }]);
		}
    ...
    methods: {
      init() {
        ...
        if ('yonghu' == this.userTableName) {
          this.dynamicProp.huiyuandengji = '初级会员,中级会员,高级会员'.split(',');
        }
        if ('yonghu' == this.userTableName) {
          this.$http.get('option/huiyuanka/huiyuanzhekou', {emulateJSON: true}).then(res => {
            if (res.data.code == 0) {
              this.dynamicProp.huiyuanzhekou = res.data.data;
			  this.$forceUpdate()
            }
          });
        }

		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="数量" prop="shuliang">
					<el-input v-model="ruleForm.shuliang" onkeyup="this.ruleForm.shuliang=this.ruleForm.shuliang.replace(/[^\d]/g,'')" placeholder="数量" clearable  :readonly="ro.shuliang"></el-input>
				</el-form-item>
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.shangpinjieshao = this.ruleForm.shangpinjieshao.replace(reg,'../../../springboot52c9713c/upload');
        } else {
          this.$message.error(data.msg);
        }
      });

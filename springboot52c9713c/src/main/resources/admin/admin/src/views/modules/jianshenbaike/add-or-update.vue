<template>
	<div class="addEdit-block" :style='{"padding":"30px","fontSize":"14px","color":"#666","background":"none"}'>
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="文章标题" prop="wenzhangbiaoti">
					<el-input v-model="ruleForm.wenzhangbiaoti"  placeholder="文章标题" clearable  :readonly="ro.wenzhangbiaoti"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="文章标题" prop="wenzhangbiaoti">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="select" v-if="type!='info'"  label="健身类型" prop="jianshenleixing">
					<el-select :disabled="ro.jianshenleixing" v-model="ruleForm.jianshenleixing" placeholder="请选择健身类型" >
						<el-option
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="健身类型" prop="jianshenleixing">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="textarea" v-if="type!='info'" label="文章简介" prop="wenzhangjianjie">
					<el-input
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else-if="ruleForm.wenzhangjianjie" label="文章简介" prop="wenzhangjianjie">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"inherit","fontWeight":"500","display":"inline-block"}'>{{ruleForm.wenzhangjianjie}}</span>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-if="type!='info'"  label="文章内容" prop="wenzhangneirong">
					<editor 
						style="min-width: 200px; max-width: 600px;"
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else-if="ruleForm.wenzhangneirong" label="文章内容" prop="wenzhangneirong">
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.wenzhangneirong = this.ruleForm.wenzhangneirong.replace(reg,'../../../springboot52c9713c/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `jianshenbaike/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jianshenbaikeCrossAddOrUpdateFlag = false;
											this.parent.search();

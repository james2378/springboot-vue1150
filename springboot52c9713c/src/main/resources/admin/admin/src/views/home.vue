<template>
<div class="content" :style='{"padding":"0px 30px 20px","alignItems":"flex-start","flexWrap":"wrap","background":"url() no-repeat center top / cover","display":"flex","fontSize":"14px","height":"100%"}'>
	<!-- notice -->
	<!-- title -->
	<div class="text" :style='{"width":"100%","margin":"-24px auto 40px","fontSize":"32px","color":"rgb(51, 51, 51)","textAlign":"center","fontWeight":"600"}'>欢迎使用 {{this.$project.projectName}}</div>
	<!-- statis -->
	<div :style='{"padding":"0","margin":"0 0 30px","alignItems":"center","color":"#fff","flexWrap":"wrap","background":"none","display":"flex","width":"100%","fontSize":"14px","justifyContent":"space-between","order":"0"}'>
		<div :style='{"boxShadow":"0 5px 20px rgba(0, 0, 0, .1)","padding":"10px","margin":"0 2% 12px 0","borderColor":"#e7e8f8","alignItems":"center","textAlign":"center","display":"flex","justifyContent":"flex-start","borderRadius":"0px","background":"#2ed8b6","borderWidth":"0px","width":"18%","borderStyle":"solid","height":"150px"}' v-if="isAuth('shangpindingdan','首页总数')">
			<div :style='{"alignItems":"center","borderRadius":"100%","background":"rgba(0, 0, 0, .2)","display":"flex","width":"65px","justifyContent":"center","height":"65px"}'>
			...
			<div :style='{"width":"auto","margin":"0 0 0 20px","textAlign":"left","flexDirection":"column-reverse","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"24px","color":"inherit","fontWeight":"500","height":"24px"}'>{{shangpindingdanCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"inherit","color":"inherit","height":"24px"}'>商品订单总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<!-- echarts -->
	<!-- 2 -->
	<div class="type2" :style='{"padding":"0","margin":"0","display":"flex","justifyContent":"space-between","alignContent":"flex-start","borderRadius":"8px","flexWrap":"wrap","background":"none","flex":"1","width":"100%","position":"relative","height":"auto","order":"10"}'>
		<div id="shangpindingdanChart1" class="echarts1" v-if="isAuth('shangpindingdan','首页统计')"></div>
		<div id="shangpindingdanChart2" class="echarts2" v-if="isAuth('shangpindingdan','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getshangpindingdanCount();
		this.shangpindingdanChat1();
		this.shangpindingdanChat2();
	},
	methods:{
		// 词云
		...
		getshangpindingdanCount() {
			this.$http({
				url: `shangpindingdan/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.shangpindingdanCount = data.data
				}
			})
		},
		shangpindingdanChat1() {
			this.$nextTick(()=>{

        var shangpindingdanChart1 = echarts.init(document.getElementById("shangpindingdanChart1"),'macarons');
        this.$http({
            url: `shangpindingdan/value/shangpinmingcheng/shuliang`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                shangpindingdanChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    shangpindingdanChart1.resize();
                };
            }
        });
      })
    ...
    shangpindingdanChat2() {
      this.$nextTick(()=>{

        var shangpindingdanChart2 = echarts.init(document.getElementById("shangpindingdanChart2"),'macarons');
        this.$http({
            url: `shangpindingdan/value/shangpinmingcheng/yue`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                shangpindingdanChart2.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    shangpindingdanChart2.resize();
                };
            }
        });
      })
